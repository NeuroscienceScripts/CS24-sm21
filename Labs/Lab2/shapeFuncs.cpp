...
{
   double res;
   res = sqrt(pow(p.x-q.x,2) + pow(p.y-q.y,2));
   return res;
   //return 99999.9; // @@@ STUB
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance)
{
   ...
   bool corner_check,dim_check;
   corner_check = assertEqual(b1.ul.x,b2.ul.x,tolerance) && assertEqual(b1.ul.y,b2.ul.y,tolerance);
   dim_check = assertEqual(b1.width,b2.width,tolerance) && assertEqual(b1.height,b2.height,tolerance);


   return corner_check && dim_check; // STUB!  TODO: Delete this line and comment and replace with appropriate code
}
